...
The function return a `%{html: html,css: css,init_props: props}`, you have to add in the html :
- the html and css
- a javascript code which uses the props string to initialize the react component

For example, if you want a page entirely generated by the react
component exported at `web/components/app.js`, add to your entry JS : 

```javascript
var App = require("./templates/counter"), React = require("react")
window.render_init = function(props,elemid){
...
EEx.eval_string("""
  ...
  <body>
    ...
    <script>render_init(<%= render.init_props %>,"content")</script>
  </body>
  </html>
